...
public class Internet4GLineService {
    ...
    public Internet4GLineResponseDTO createInternet4GLine(Internet4GLineCreateRequestDTO createRequestDTO) {
        if (internet4GLineRepository.existsByLineNumber(createRequestDTO.getLineNumber())) {
            throw new ResourceAlreadyExistsException("4G Line with number " + createRequestDTO.getLineNumber() + " already exists.");
        }

        if (internet4GLineRepository.existsBySimSerialNumber(createRequestDTO.getSimSerialNumber())) {
            throw new ResourceAlreadyExistsException("SIM serial number " + createRequestDTO.getSimSerialNumber() + " already exists.");
        }

        if (internet4GLineRepository.existsByEquipmentSerialNumber(createRequestDTO.getEquipmentSerialNumber())) {
            throw new ResourceAlreadyExistsException("Equipment serial number " + createRequestDTO.getEquipmentSerialNumber() + " already exists.");
        }

        Internet4GLine internet4GLine = internet4GLineMapper.toEntity(createRequestDTO);
    ...
    public Internet4GLineResponseDTO updateInternet4GLine(UUID id, Internet4GLineUpdateRequestDTO updateRequestDTO) {
        Internet4GLine internet4GLine = internet4GLineRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("4G Line with id " + id + " not found."));

        // uniqueness checks when new values provided
        if (updateRequestDTO.getSimSerialNumber() != null
            && !updateRequestDTO.getSimSerialNumber().equals(internet4GLine.getSimSerialNumber())
            && internet4GLineRepository.existsBySimSerialNumber(updateRequestDTO.getSimSerialNumber())) {
            throw new ResourceAlreadyExistsException("SIM serial number " + updateRequestDTO.getSimSerialNumber() + " already exists.");
        }
        if (updateRequestDTO.getEquipmentSerialNumber() != null
            && !updateRequestDTO.getEquipmentSerialNumber().equals(internet4GLine.getEquipmentSerialNumber())
            && internet4GLineRepository.existsByEquipmentSerialNumber(updateRequestDTO.getEquipmentSerialNumber())) {
            throw new ResourceAlreadyExistsException("Equipment serial number " + updateRequestDTO.getEquipmentSerialNumber() + " already exists.");
        }

        internet4GLineMapper.updateEntityFromDto(updateRequestDTO, internet4GLine);

        if (updateRequestDTO.getLineNumber() != null && !updateRequestDTO.getLineNumber().equals(internet4GLine.getLineNumber())
            && internet4GLineRepository.existsByLineNumber(updateRequestDTO.getLineNumber())) {
            throw new ResourceAlreadyExistsException("4G Line with number " + updateRequestDTO.getLineNumber() + " already exists.");
        }
        ...
        if (updateRequestDTO.getCreatedBy() != null) {
            internet4GLine.setCreatedBy(updateRequestDTO.getCreatedBy());
        }

        Internet4GLine updatedLine = internet4GLineRepository.save(internet4GLine);
        return internet4GLineMapper.toInternet4GLineResponseDTO(updatedLine);
    }

    public void terminatedInternet4GLine(UUID id) {
        Internet4GLine internet4GLine = internet4GLineRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("4G Line with id " + id + " not found."));
        if (internet4GLine.getLineStatus() != null && internet4GLine.getLineStatus().equals(LineStatus.ACTIVE)) {
            internet4GLine.setLineStatus(LineStatus.TERMINATED);
    ...
    public void deleteInternet4GLine(UUID id) {
        Internet4GLine internet4GLine = internet4GLineRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("4G Line with id " + id + " not found."));
        if (internet4GLine.getLineStatus() != null && internet4GLine.getLineStatus().equals(LineStatus.ACTIVE)) {
            throw new IllegalStateException("Cannot delete an active 4G Line.");
        }
        internet4GLineRepository.delete(internet4GLine);
    }

    public List<Internet4GLineResponseDTO> getAllInternet4GLines() {
        return internet4GLineRepository.findAll().stream()
    ...
    public Internet4GLineResponseDTO getInternet4GLineById(UUID id) {
        Internet4GLine internet4GLine = internet4GLineRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("4G Line with id " + id + " not found."));
        return internet4GLineMapper.toInternet4GLineResponseDTO(internet4GLine);
    }

    public Internet4GLineResponseDTO getInternet4GLineByLineNumber(String lineNumber) {
        Internet4GLine internet4GLine = internet4GLineRepository.findByLineNumber(lineNumber)
            .orElseThrow(() -> new ResourceNotFoundException("4G Line with number " + lineNumber + " not found."));
        return internet4GLineMapper.toInternet4GLineResponseDTO(internet4GLine);
    }

    public List<Internet4GLineResponseDTO> getAllInternet4GLinesByStatus(LineStatus lineStatus) {
        return internet4GLineRepository.findByLineStatus(lineStatus).stream()
                .map(internet4GLineMapper::toInternet4GLineResponseDTO)
                .collect(Collectors.toList());
    }

    public List<Internet4GLineResponseDTO> getAllBillableInternet4GLines(){
        List<Internet4GLineResponseDTO> internet4GLines = getAllInternet4GLinesByStatus(LineStatus.ACTIVE);
        internet4GLines.addAll(getAllInternet4GLinesByStatus(LineStatus.SUSPENDED));
        return internet4GLines;
    }

    public List<Internet4GLineResponseDTO> searchInternet4GLines(String lineNumber, LineStatus lineStatus,
                                                                 String serviceFunction, String simSerialNumber,
                                                                 String pinCode, String pukCode,
                                                                 String equipment, String equipmentSerialNumber,
                                                                 String bandwidth) {
        Specification<Internet4GLine> spec = Specification
                .<Internet4GLine>where(LineSpecification.hasLineNumber(lineNumber))
        ...
        return internet4GLineRepository.findAll(spec).stream()
                .map(internet4GLineMapper::toInternet4GLineResponseDTO)
                .collect(Collectors.toList());
    }
